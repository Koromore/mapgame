...
gulp.task("prepare",["clean"],function () {
    if (projectInfo.type == "canvas") {
        ...
        //压缩main.js
        gulp.src("src/Main.js").pipe(uglify()).pipe(rename("f2xMain.min.js")).pipe(gulp.dest("released/src"));
        //压缩各个scene
        for (var i = 0; i < sceneList.length; i++) {
            ...
            //合并压缩js
            gulp.src(jsList).pipe(uglify()).pipe(concat(sceneList[i] + ".swf")).pipe(gulp.dest("released/src/" + sceneList[i]));
            //复制其他资源
